...
class TaskRunner:
    ...
    def run(self, config):
        ...
        # 🔥 为混合训练添加进度管理和辅助数据处理（修正版）
        hybrid_training_manager = None
        if config.data.get("data_source") == "sales_rag_hybrid":
            # 创建修正版混合训练管理器，支持组内中心化
            from verl.workers.hybrid_grpo_training_manager import HybridTrainingManager
            
            hybrid_training_manager = HybridTrainingManager(
                enable_auxiliary_reward=config.algorithm.hybrid_grpo.get("enable", True),
                auxiliary_weight=config.algorithm.hybrid_grpo.get("auxiliary_weight", 0.3),
                weight_decay_rate=config.algorithm.hybrid_grpo.get("weight_decay_rate", 0.4),
                min_auxiliary_weight=config.algorithm.hybrid_grpo.get("min_auxiliary_weight", 0.1),
                scoring_model=config.algorithm.hybrid_grpo.get("scoring_model", "GPT-5"),
                group_size=config.algorithm.hybrid_grpo.get("group_size", 5),
                enable_dynamic_weight=config.algorithm.hybrid_grpo.get("enable_dynamic_weight", True)
            )
            
            print_rank_0(f"✅ 加载修正版混合训练管理器")
            print_rank_0(f"🎯 训练目标：保持GRPO组内相对优化，GPT-5提供质量信号")
