...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        Args:
            ...
            dual_model_processors: Optional dictionary containing dual model processors for SalesRAG.
        """
        ...
        # 🔥 初始化双模型处理器（用于SalesRAG）
        self.dual_model_processors = dual_model_processors
    ...
    async def _process_dual_models_batch(self, batch: DataProto) -> tuple[DataProto, dict]:
        """
        处理双模型批次（用于SalesRAG）
        ...
        Returns:
            tuple: (处理后的批次, 额外信息字典）
        """
        if not self.dual_model_processors:
            return batch, {}
        
        try:
            logger.info("开始双模型批次处理")
            
            # 提取样本数据
            samples = []
            for i in range(len(batch)):
                sample_data = {
                    "prompt_id": batch.non_tensor_batch[i].get("prompt_id", f"sample_{i}"),
                    "prompt": self.tokenizer.decode(batch.batch["prompts"][i], skip_special_tokens=True),
                    "parsed_data": batch.non_tensor_batch[i].get("parsed_data", {})
                }
                samples.append(sample_data)
            
            # 并行处理Actor和参考模型
            actor_manager = self.dual_model_processors["actor_manager"]
            reference_manager = self.dual_model_processors["reference_manager"]
            
            actor_results = await actor_manager.process_training_batch(samples)
            reference_results = await reference_manager.process_training_batch(samples)
            
            # 计算奖励
            rewards, statistics = await self.reward_fn(actor_results, reference_results)
            
            # 更新批次数据
            for i, (actor_result, reference_result, reward) in enumerate(
                zip(actor_results, reference_results, rewards)
            ):
                # 将结果存储回批次
                batch.non_tensor_batch[i]["actor_result"] = actor_result
                batch.non_tensor_batch[i]["reference_result"] = reference_result
                batch.non_tensor_batch[i]["dual_model_reward"] = reward
            
            logger.info(f"双模型批次处理完成，奖励统计: {statistics}")
            return batch, {"reward_statistics": statistics}
